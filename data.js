...
const CV = {
  ...
  projects: [
    ...
    },
    {
      ...
      featured: true,
      desc: "A parcel-logistics operator watches roughly 120 vehicles across 8 depots and 15 routes live, rather than the next morning. GPS pings and depot scan events stream through a Fabric Eventstream into OneLake, where three Materialized Lake Views declare Silver and five declare Gold — the entire transformation layer is eight declarative views, with no hand-orchestrated Spark ETL touching the shipment-status or route-performance tables. A scoring notebook grades every active shipment's late-delivery risk on a strict train/score split, so the injected truth built into the story never leaks into the features that predict it. A Direct Lake model and a five-page report read the same Gold tables the ontology binds to, so when a Data Agent turned out unsupported on this tenant's trial SKU, an OperationsAgent substituted in the same session at zero build cost. The whole estate — Eventstream, Lakehouse, Variable Library, semantic model — deploys from a Git-integrated folder of item definitions in one workspace. The failures are published too: a live-replay leg of the pipeline posts cleanly but is never reflected in Bronze, and the platform's own hypothesis about whether this deployment approach is faster than its predecessor could not be tested, because the channel it depends on was never reachable.",
